...
namespace SubscriptionService.Repository
{
    public class SubscribeDrugs : ISubscribeDrugs
    {
        ...
        public SubscriptionDetails PostSubscribe(PrescriptionDetails prescription)
        {
            ...
            // Drug drug = new Drug();
            // var a = prescription.DrugName;
            /*  var query = "?name=" + prescription.DrugName;
              HttpClient client = new HttpClient();
              HttpResponseMessage result = client.GetAsync("https://localhost:44393/DrugApi?" + prescription.DrugName).Result;


              if (result.IsSuccessStatusCode)
              {
                  drug = result.Content.ReadAsStringAsync().Result;
            ...
            foreach (var sub in details)
            {
                ...
                if (sub.Id == id)
                {
                    ...
                    if (refill.Id == id && refill.Payment == true)
                    {
                        ...
                        return new SubscriptionDetails { Id = 1, MemberId = 1, MemberLocation = "Delhi", PrescriptionId = 101, RefillOccurrence = "weekly", Status = true, SubscriptionDate = Convert.ToDateTime("2020-12-01 01:01:00 AM") };
